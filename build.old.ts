...
function isolateComponents({ segments, exports }: StreamNode): TokensTree<ComponentToken> {
  const tokensTree: TokensTree<ComponentToken> = { components: {}, exports }
...
type tagMapper = (tag: string, attributes?: string) => { open: string, close?: string }
const openClose: tagMapper = (tag, attributes = '') => ({ open: `<${tag}${attributes}>`, close: `</${tag}>` })
const openCloseInline: tagMapper = (tag, attributes = '') => ({ open: `<${tag}${attributes}/>` })
...
function buildComponentSegment(node: SegmentNode): string {
  ...
  return htmlSnipet.open + childrenSnipet + (htmlSnipet.close ?? '')
}
...
function buildHtml(tree: TokensTree<ComponentTokenBuild>): string {
  ...
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta http-equiv="X-UA-Compatible" content="IE=edge"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Document</title></head><body>${appContent}</body></html>`
}
